...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] font-sans text-[var(--text-primary)] antialiased py-8 px-4 sm:px-6 sm:py-12">
      <div className="max-w-2xl mx-auto">
        {/* Header Section */}
        <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-[var(--color-brand-coral)] flex items-center justify-center shadow-[0_0_20px_rgba(218,119,86,0.2)]">
              <svg
          ...
          <div className="flex items-center gap-4 bg-[var(--bg-card)] px-4 py-2 rounded-2xl border border-[var(--border)] shadow-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-[var(--text-muted)] truncate max-w-[140px]" title={user?.email}>
                {user?.email}
              </span>
            </div>
        ...
        {/* Main Content Card */}
        <main className="bg-[var(--bg-card)] rounded-[2rem] border border-[var(--border)] shadow-2xl shadow-black/10 overflow-hidden">
          
          {/* Input Area */}
          <div className="p-6 sm:p-8 border-b border-[var(--border)] bg-[var(--bg-card)] relative z-10">
            <form onSubmit={addTodo}>
              <div className="flex gap-3">
                <input
                  ...
                  className="flex-1 rounded-2xl border border-[var(--border)] bg-[var(--bg-input)] px-5 py-4 text-base placeholder:text-[var(--text-muted)]/50 focus:outline-none focus:ring-2 focus:ring-[var(--color-brand-coral)]/30 focus:border-[var(--color-brand-coral)] transition-all duration-300 shadow-inner"
                />
                <button
                  type="submit"
                  disabled={adding || !newTask.trim()}
                  className="rounded-2xl bg-[var(--color-brand-coral)] px-7 py-4 text-[#FEFDFB] font-semibold hover:bg-[var(--color-brand-coral-hover)] focus:outline-none focus:ring-2 focus:ring-[var(--color-brand-coral)]/50 focus:ring-offset-2 focus:ring-offset-[var(--bg-card)] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_4px_14px_rgba(218,119,86,0.3)] hover:shadow-[0_6px_20px_rgba(218,119,86,0.4)] cursor-pointer whitespace-nowrap active:scale-[0.98]"
                >
                  {adding ? (
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8">
              <div className="flex p-1.5 bg-[var(--bg-input)] border border-[var(--border)] rounded-full w-full sm:w-auto">
            ...
            {loadingTodos ? (
              ...
              <ul className="space-y-3">
                {filteredTodos.map((todo) => (
                  <li
                    key={todo.id}
                    className="group flex items-center gap-4 bg-[var(--bg-input)] border border-[var(--border)] rounded-2xl px-5 py-4 hover:border-[var(--color-brand-coral)]/40 hover:shadow-md transition-all duration-200"
                  >
                    {/* Custom Checkbox */}
                    <button
